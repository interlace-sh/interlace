...
| **models** | filterable catalog (name, language, output, strategy, engine, tags, schedule); detail: fingerprint (click to copy), column lineage, declared indexes and constraints (and the external `schema` policy on a terminal table), upstream/downstream, SQL or Python source, latest checks | **trace in lineage**, **run** (`POST /runs`), **query**, per-column **impact** (`GET /models/{name}/impact`) |
| **plan** | per-change cards with category pills, impacted columns, SQL diffs, an indexes-and-constraints card (`+ index` / `- constraint`), drift notes, cross-engine transfers, breaking warning; apply results (built/reused/promoted, row deltas, timings). A physical-only plan is not "nothing to do" | environment field, selector input, **changed only** (`state:modified+`), forward-only, **preview** (`GET /plan`), **apply** (`POST /apply`) with a breaking-change confirm modal |
