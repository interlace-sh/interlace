...
| **lineage** | the whole DAG (one `/lineage` payload) with per-node output/strategy/engine, schedule ⏱ / checks ✓ marks, expandable column pins with types; streams as source nodes. The selected model shows its last status, row delta, duration, failed statement, column profile, and a row sample | search/focus a model, expand columns, click a column to trace it through the graph, edges flow live while models build |
| **models** | filterable catalog (name, language, output, strategy, engine, tags, schedule); detail: fingerprint (click to copy), column lineage, declared indexes and constraints (and the external `schema` policy on a terminal table), upstream/downstream, SQL or Python source, latest checks, a row sample and column profile | **trace in lineage**, **run** (`POST /runs`), **query**, per-column **impact** (`GET /models/{name}/impact`), preview (`GET /models/{name}/preview`) |
