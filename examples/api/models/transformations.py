...
async def user_activity_summary(users: ibis.Table, posts: ibis.Table, todos: ibis.Table, carts: ibis.Table):
    ...
    # Join with users on id == user_id, dropping right-side join keys
    base = users[["id", "username", "email", "first_name", "last_name", "age", "gender"]]
    joined = base.left_join(posts_by_user, base.id == posts_by_user.user_id).drop("user_id")
    joined = joined.left_join(todos_by_user, joined.id == todos_by_user.user_id).drop("user_id")
    joined = joined.left_join(carts_by_user, joined.id == carts_by_user.user_id).drop("user_id")
