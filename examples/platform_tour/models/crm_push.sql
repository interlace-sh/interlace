...
-- never dropped; only changed keys are touched. Sinks are environment-gated
-- (default: prod only) so a sandbox apply never fires at a live destination;
-- this tour opts dev in because its "CRM" is a local file.
