...
class Config:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.data, dict):
            errors.append(
                f"Configuration must be a dictionary/mapping, got {type(self.data).__name__}"
            )
            raise ValueError("\n".join(errors))
        ...
        if connections is not None and not isinstance(connections, dict):
            errors.append(
                f"Configuration 'connections' must be a dictionary, got {type(connections).__name__}"
            )
...
def load_config(project_path: Optional[Path] = None, env: Optional[str] = None) -> Config:
    """
    ...
    if not base_config_path.is_file():
        raise FileNotFoundError(
            f"Configuration path is not a file: {base_config_path}"
        )
        ...
        with open(base_config_path, 'r') as f:
            try:
                ...
                if hasattr(e, 'problem_mark'):
                    ...
                    raise ValueError(
                        f"Error parsing config.yaml: {error_msg}\n"
                        f"  File: {base_config_path}"
                    ) from e
