...
class ResourceMonitor:
    ...
    def check_execution_time(self, start_time: datetime) -> Dict[str, Any]:
        ...
        if self.max_execution_time_seconds and elapsed > self.max_execution_time_seconds:
            result["within_limit"] = False
            logger.warning(
                f"Execution time ({elapsed:.1f}s) exceeds limit "
                f"({self.max_execution_time_seconds}s)"
            )
    ...
    def track_connection(self, action: str = "open") -> Dict[str, Any]:
        ...
        if self.max_connections and self._connection_count > self.max_connections:
            result["within_limit"] = False
            logger.warning(
                f"Connection count ({self._connection_count}) exceeds limit "
...
class ConnectionPoolMetrics:
    ...
    def get_pool_metrics(self, name: str) -> Optional[Dict[str, Any]]:
        """
...
def timed(func: Optional[Callable] = None, name: Optional[str] = None, log: bool = False):
    """
...
class PerformanceMonitor:
    ...
    def get_report(self) -> Dict[str, Any]:
        ...
        total_model_time = sum(
            sum(times) for times in model_times.values()
        )
        avg_model_time = (
            total_model_time / len(model_times) if model_times else 0
        )
        ...
        return {
            ...
            "dependency_loading": {
                ...
                "average_time_seconds": (
                    sum(self.metrics["dependency_loading_times"].values()) / len(self.metrics["dependency_loading_times"])
                    if self.metrics["dependency_loading_times"] else 0
                ),
            },
            "database_queries": {
                ...
                "average_duration_seconds": (
                    ...
                    if self.metrics["database_queries"]["count"] > 0 else 0
                ),
            },
