...
class TestAPIContextManager:
    ...
    async def test_context_manager_handles_auth_failure(self):
        ...
        async with api:
            ...
            assert (
                "Authorization" not in api.session.headers
                or api.session.headers.get("Authorization") != "Bearer token123"
            )
...
class TestAPIPagination:
    ...
    async def test_paginated_custom_count_attribute(self):
        ...
        with aioresponses() as m:
            ...
            async with API(base_url="https://api.example.com") as api:
                result = await api.paginated("/users", page_size=100, count_attribute="pagination.total")
...
class TestAPIAuthHelpers:
    ...
    async def test_basic_auth_token(self):
        ...
        with aioresponses() as m:
            m.post("https://auth.example.com/auth", payload={"token": "api_token_123"})
    ...
    async def test_basic_auth_token_with_access_token(self):
        ...
        with aioresponses() as m:
            m.post("https://auth.example.com/auth", payload={"access_token": "token123"})
    ...
    async def test_incremental_loading_pattern(self):
        ...
        with aioresponses() as m:
            ...
            m.get(
                ...
                payload={
                    "meta": {"count": 50},
                    "data": [{"id": i, "created_at": f"2024-01-{i:02d}"} for i in range(1, 51)],
                },
            )
